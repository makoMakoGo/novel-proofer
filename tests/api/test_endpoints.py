...
def test_job_actions_pause_resume(monkeypatch: pytest.MonkeyPatch):
    ...
    try:
        ...
        execution = GLOBAL_EXECUTIONS.begin(job2.job_id, "process")
...
def test_job_snapshot_contract_covers_running_user_paused_and_cancelled() -> None:
    ...
    try:
        ...
        execution = GLOBAL_EXECUTIONS.begin(job.job_id, "process")
        GLOBAL_EXECUTIONS.mark_running(execution.attempt_id)
        ...
        GLOBAL_EXECUTIONS.finish(execution.attempt_id)
        GLOBAL_JOBS.mark_reset_requested(job.job_id)
...
def test_job_list_uses_snapshot_contract_without_legacy_state_fields() -> None:
    ...
    try:
        ...
        assert item.get("execution_state") == "idle"
...
def test_reset_completed_job_deletes_without_forcing_cancel(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fail_mark_reset_requested(job_id: str, **_kwargs: object) -> bool:
        raise AssertionError(f"mark_reset_requested should not be called for completed reset: {job_id}")

    monkeypatch.setattr(api.GLOBAL_JOBS, "mark_reset_requested", fail_mark_reset_requested)
...
def test_resume_job_returns_409_when_background_submit_rejects(monkeypatch: pytest.MonkeyPatch):
    ...
    try:
        GLOBAL_JOBS.update(job.job_id, phase="process")
        assert GLOBAL_JOBS.mark_execution_stopped(job.job_id, phase="process") is True
...
def test_resume_rejects_duplicate_active_execution() -> None:
    client = TestClient(api.app)
    job = GLOBAL_JOBS.create("in.txt", "out.txt", total_chunks=0)
    execution = GLOBAL_EXECUTIONS.begin(job.job_id, "process")
    try:
        ...
        assert r.status_code == 409, r.text
        assert "already has active execution" in r.text
        st = GLOBAL_JOBS.get(job.job_id)
        assert st is not None
        assert st.state == "paused"
...
def test_resume_worker_crash_reconciles_error_and_clears_execution(monkeypatch: pytest.MonkeyPatch) -> None:
    client = TestClient(api.app)
    job = GLOBAL_JOBS.create("in.txt", "out.txt", total_chunks=0)
    ...
    monkeypatch.setattr(api, "resume_paused_job", crash)
    try:
        assert GLOBAL_JOBS.mark_execution_stopped(job.job_id, phase="process") is True
        ...
        body = _wait_job_done(client, job.job_id, timeout_seconds=5.0)
        _assert_snapshot(body, workflow_phase="process", terminal_state="error")
        st = GLOBAL_JOBS.get(job.job_id)
        assert st is not None
        assert st.error == "process worker crashed: boom"
        assert GLOBAL_EXECUTIONS.get(job.job_id) is None
    finally:
        GLOBAL_JOBS.delete(job.job_id)


def test_reset_active_execution_requests_delete_and_cleans_after_done(monkeypatch: pytest.MonkeyPatch) -> None:
    client = TestClient(api.app)
    with tempfile.TemporaryDirectory() as td:
        base = Path(td)
        out_dir = base / "output"
        ...
        paths._write_input_cache(job.job_id, "cached")
        GLOBAL_JOBS.update(
            job.job_id,
            state="running",
            phase="process",
        ...
        try:
            ...
            assert not paths._input_cache_path(job.job_id).exists()
        finally:
            GLOBAL_EXECUTIONS.finish(execution.attempt_id)
            GLOBAL_JOBS.delete(job.job_id)

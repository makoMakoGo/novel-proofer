...
1) **开始校验**：解码、切片、确定性规则预处理，生成可恢复任务快照；完成后任务会暂停，等待下一步。
...
- 任务若 `execution_state=queued/running`：说明当前后端进程仍有执行尝试，可以查看状态与进度；process 阶段可显式暂停，也可直接删除任务。
...
- 「已加载任务」状态下右侧的“字数”只来自服务端输入缓存（`output/.inputs/{job_id}.txt`）。如果缓存缺失，接口会明确报错，UI 会显示 `-` 或 “读取失败”。
- 刷新/关闭页面只会停止当前浏览器页面的观察者，不会暂停、删除、取消或 abort 后端任务。重新打开页面会尝试重新关联上次加载的任务并展示当前快照；要停止校对，请显式点击「暂停」或「删除任务」。

目录/记录的含义：
- `output/.jobs/{job_id}/`：分片级调试产物（`out`；`resp` 为可选；`pre/` 仅保留目录结构），用于排障与复盘
...
| 暂停 | 向当前执行发送 stop 请求，并将任务置为 `paused`（仅 process 可用） | 不删除 | 可继续校对；可刷新/重启后继续 |
...
| 合并后清理中间产物（output/.jobs） | 合并成功后删除 `output/.jobs/{job_id}/`；删除失败会显式报错 | `output/.jobs/{job_id}/` | 不影响下载最终输出、不影响加载任务查看状态；但无法再用 `out`（以及可选的 `resp`）排查/复盘该次运行 |
...
| 删除任务（未完成） | 向当前执行发送 delete 请求，并在执行结束后删除该任务 | `output/.jobs/{job_id}/`、`output/.inputs/{job_id}.txt`、`output/.state/jobs/{job_id}.json`，并从任务列表删除 | 任务不可再加载/继续；不会删除 `output/` 下已生成的最终输出文件（如果存在） |
...
- 如果你仅合并完成（且勾选了“合并后清理中间产物（output/.jobs）”），`output/.jobs/{job_id}/` 可能会被自动清理，但输入缓存仍会保留，以便后续重跑全部、重启后继续处理或重试失败分片
- 如果输入缓存缺失，这些能力会明确失败；系统不会回退到调试目录推断输入内容

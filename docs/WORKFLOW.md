...
- **刷新/关闭页面**：当前前端会在 `pagehide` / `beforeunload` 中调用 `api.bestEffortPauseJob(state.currentJobId)`，向 `/api/v1/jobs/{job_id}/pause` 发送一次 best-effort 暂停请求；如果请求没送达，任务就会继续运行。这个 browser lifecycle pause 行为会在后续 `#77` 中移除，以对齐 `docs/WORKFLOW_RECOVERY_REFACTOR.md` 的目标架构。

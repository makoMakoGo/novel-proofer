...
class LLMConfig:
    ...
    extra_params: dict[str, Any] | None = None  # Optional JSON passthrough for thinking config etc.
...
6. 清理垃圾内容：删除广告/引流/水印/群链接等垃圾信息；删除仅由 = - * _ — 等重复符号组成的分隔线（如 ====、----、***、________、———）。如果某行包含正文内容（含汉字/叙述/对话），不要误删
